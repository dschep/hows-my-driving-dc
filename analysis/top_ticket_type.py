...
tweet_df.drop_duplicates(['state', 'number'], inplace=True)
...
for state in dmv_df['state'].drop_duplicates().tolist():
    ...
    out_state_df = (state_df['violation'].value_counts().head(5) / len(state_df)).to_frame(name=state)
    out_state_df.to_csv('output/{}_top_5.csv'.format(state))
